...
function dot({ order, code, description, borough, street, side, from, to,
  orderType = 'P-', onSuffix = null, fromSuffix = null, toSuffix = null }) {
  return {
    order_number: order, record_type: 'Current', order_type: orderType, borough,
    on_street: street, on_street_suffix: onSuffix,
    from_street: from, from_street_suffix: fromSuffix,
    to_street: to, to_street_suffix: toSuffix, side_of_street: side,
...
const fixtures = [
  {
    ...
    expectedRelationship: 'WHOLE_FACE', expected: { cleaning: 'UNKNOWN', meter: 'UNKNOWN' },
  ...
  },
  {
    ...
    expectedRelationship: 'WHOLE_FACE', expected: { cleaning: 'SUPPORTED', meter: 'SUPPORTED' },
  ...
  },
  {
    ...
    expectedRelationship: 'WHOLE_FACE', expected: { cleaning: 'CAUTION', meter: 'CAUTION' },
    officialRoadwayGeometryComplete: false,
    currentSourceNote: 'The retained public CSCL excerpt does not cover the full ParkNYC block-face geometry, so meter geometry remains unverified CAUTION.',
  ...
  },
  {
    ...
    expectedRelationship: 'UNKNOWN', expected: { cleaning: 'UNKNOWN', meter: 'CAUTION' },
    officialRoadwayGeometryComplete: false,
    currentSourceNote: 'The exact reviewed nfid-uabd query returned no current row, and the retained CSCL excerpt does not cover the full ParkNYC geometry; cleaning remains UNKNOWN and meter geometry remains unverified CAUTION.',
    dotRows: [],
  ...
  },
  {
    ...
    expectedRelationship: 'UNKNOWN', expected: { cleaning: 'UNKNOWN', meter: 'CAUTION' },
    officialRoadwayGeometryComplete: false,
    currentSourceNote: 'Official DOT order P-01683639 carries on_street_suffix W RDWY, which remains UNKNOWN until an authoritative roadbed translation exists; retained CSCL evidence also leaves meter geometry unverified CAUTION.',
    dotRows: [dot({ order: 'P-01683639', code: 'PS-161B', description: 'NO PARKING (SANITATION BROOM SYMBOL) MONDAY 12AM-3AM <->', borough: 'STATEN ISLAND', street: 'BAY STREET', side: 'W', from: 'VICTORY BOULEVARD', to: 'HANNAH STREET', onSuffix: 'W RDWY' })],
  ...
  },
  {
    ...
    expectedRelationship: 'UNKNOWN', expected: { cleaning: 'UNKNOWN', meter: 'UNKNOWN' },
